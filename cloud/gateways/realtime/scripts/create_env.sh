#!/usr/bin/env bash

# Get the app path
env_path=$(dirname $(dirname "$0"))/.env

# create .env file
{
  echo "LOG_LEVEL=DEBUG"
  echo "GUNICORN_WORKERS=1"
  echo "SENTRY_DSN="
  echo "SENTRY_TRACE_SAMPLE_RATE=1.0"
  echo "SENTRY_PROFILES_SAMPLE_RATE=1.0"
  echo "ENVIRONMENT=local"
  echo "SIBEL_VERSION=local"
  echo "APPLICATION_PORT=7007"
  echo "BASE_PATH=/realtime"
  echo "JWT_VERIFYING_KEY="
  echo "BROKER_TOPIC_VITALS=vitals"
  echo "BROKER_TOPIC_ALERTS=alerts"
  echo "BROKER_TOPIC_PATIENTS=events-patient"
  echo "BROKER_TOPIC_DEVICES=events-device"
  echo "BROKER_TOPIC_SDC_REALTIME_STATE=sdc-realtime-state"
  echo "BROKER_TOPIC_SDC_EVENTS=events-sdc"
  echo "KAFKA_HOST=localhost"
  echo "KAFKA_PORT=9092"
  echo "KAFKA_PASSWORD=cantguessthis"
  echo "KAFKA_CA_FILE_PATH=''"
  echo "KAFKA_CERT_FILE_PATH=''"
  echo "KAFKA_KEY_FILE_PATH=''"
  echo "KAFKA_RETRY_BACKOFF=1000"
  echo "KAFKA_METADATA_MAX_AGE=300000"
  echo "WEB_BASE_URL=http://localhost:8000/web"
  echo "CORS_ORIGINS=http://localhost:3000"
  echo "KAFKA_HEALTHCHECK_TOPIC=healthcheck"
} > $env_path

echo "Local $env_path file created."
