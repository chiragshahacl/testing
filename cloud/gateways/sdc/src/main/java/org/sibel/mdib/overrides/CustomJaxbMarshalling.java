 ...
 */
public class CustomJaxbMarshalling extends AbstractIdleService {
    private static final Logger LOG = LogManager.getLogger(org.somda.sdc.dpws.helper.JaxbMarshalling.class);
    ...
    @Inject
    public CustomJaxbMarshalling(
            @Named(SoapConfig.JAXB_CONTEXT_PATH) String contextPackages,
        ...
        this.namespacePrefixMapper =
                namespacePrefixMapperConverter.convert(namespaceMappingParser.parse(namespaceMappingsExtended));
    ...
    private void initializeJaxb() throws SAXException, IOException, ParserConfigurationException {
        ...
        contextPackages += PKG_DELIM;
        contextPackages += MdpwsConfig.MDPWS_CONTEXT_PATH;
        ...
        if (validateSoapMessages) {
            var extendedSchemaPath = SoapConstants.SCHEMA_PATH
                    + SCHEMA_DELIM
                    + WsAddressingConstants.SCHEMA_PATH
                    + SCHEMA_DELIM
                    + WsDiscoveryConstants.SCHEMA_PATH
                    + SCHEMA_DELIM
                    + WsEventingConstants.SCHEMA_PATH
                    + SCHEMA_DELIM
                    + WsMetadataExchangeConstants.SCHEMA_PATH
                    + SCHEMA_DELIM
                    + WsTransferConstants.SCHEMA_PATH
                    + SCHEMA_DELIM
                    + DpwsConstants.SCHEMA_PATH
                    + SCHEMA_DELIM
                    + WsdlConstants.SCHEMA_PATH
                    + SCHEMA_DELIM
                    + MdpwsConfig.MDPWS_SCHEMA_PATH
                    + SCHEMA_DELIM
                    + schemaPath;
    ...
    private Schema generateTopLevelSchema(String schemaPath)
        ...
        for (String path : schemaPath.split(SCHEMA_DELIM)) {
            ...
            instanceLogger.info("Register namespace for validation: {}, read from {}", targetNamespace, schemaUrl);
