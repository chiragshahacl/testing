...
from app.common.kafka_admin import KafkaAdminClientFactory
...
def database_exists(connection: Connection) -> bool:
    result = connection.execute(
        text("SELECT datname FROM pg_catalog.pg_database WHERE datname = 'test_config'")
    )
...
def clean_database(_: Context) -> None:
    ...
    with engine.connect() as connection:
        if database_exists(connection):
            connection.execute(text("REVOKE CONNECT ON DATABASE test_config FROM public;"))
            connection.execute(
                text(
                    """
                SELECT pid, pg_terminate_backend(pid)
                FROM pg_stat_activity
                WHERE datname = 'test_config' AND pid <> pg_backend_pid();
                """
                )
            )
            connection.execute(text("DROP DATABASE IF EXISTS test_config;"))
        connection.execute(text("CREATE DATABASE test_config"))
        connection.execute(text("GRANT CONNECT ON DATABASE test_config TO public;"))
    run_migrations_up_to("head")
    importlib.reload(database_module)


@fixture
def mock_kafka_admin_client(_: Context) -> None:
    kafka_admin_mock = mock.Mock()
    kafka_admin_mock.alter_configs.return_value = {"topic": mock.Mock()}
    kafka_admin_mock.create_topics.return_value = {"topic": mock.Mock()}
    with mock.patch("app.common.kafka_admin.AdminClient", return_value=kafka_admin_mock):
        yield kafka_admin_mock
...
def before_scenario(context, _):
    ...
    use_fixture(mock_kafka_admin_client, context)
