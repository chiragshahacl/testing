...
def database_exists(connection: Connection) -> bool:
    result = connection.execute(
        text("SELECT datname FROM pg_catalog.pg_database WHERE datname = 'test_patient'")
    )
...
def clean_database(_: Context) -> None:
    ...
    with engine.connect() as connection:
        if database_exists(connection):
            connection.execute(text("REVOKE CONNECT ON DATABASE test_patient FROM public;"))
            connection.execute(
                text(
                    """
                SELECT pid, pg_terminate_backend(pid)
                FROM pg_stat_activity
                WHERE datname = 'test_patient' AND pid <> pg_backend_pid();
                """
                )
            )
            connection.execute(text("DROP DATABASE IF EXISTS test_patient;"))
        connection.execute(text("CREATE DATABASE test_patient"))
        connection.execute(text("GRANT CONNECT ON DATABASE test_patient TO public;"))
    run_migrations_up_to("head")
    importlib.reload(database_module)


@fixture
def mock_dt_freeze(context):
...
@fixture
def mock_dt(context):
    now = datetime(2023, 8, 16)
    with mock.patch("app.common.event_sourcing.publisher.datetime") as dt_mock:
...
def before_all(context):
    use_fixture(mock_dt_freeze, context)
